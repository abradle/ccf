...
class FragScore(models.Model):
    ...
    score = models.FloatField()
    type = models.CharField(max_length=30)
    frag_id = models.ForeignKey(MMPFrag)
...
class KeyCluster(models.Model):
    ...
    pharma_id = models.ManyToManyField(PharmaPoint)
    # The MMPFrag associate to it
    mmp_frag_id = models.ManyToManyField(MMPFrag)
    ...
    # The molecules associated to it
    actpharma_id = models.ManyToManyField(ActPharmaPoint)
    # The molecules associated to it
    act3dfrag_id = models.ManyToManyField(Act3DFrag)
...
#class TargetFPS(models.Model):
#    """Model to store the JSON for a targets fingerprint data
#    At first just store raw JSON - after a while we can use this to manipulate
#    and store different options"""
#    # The target it relates to
#    target_id = models.ForeignKey(Target)
#    # The info
